MODULE processes
    ...
    PROC setup_EGM()
        ...
        IF egmSt1 <= EGM_STATE_CONNECTED THEN
            EGMSetupUC ROB_L, egmID1, "default", "EGMsensor:"\pose;
            !EGMConfig egmCfg, \Pose, \CondTime:=5, \CommTimeout:=10;
        ENDIF

        EGMActPose egmID1 \Tool:=tGripper,corr_frame_offs,EGM_FRAME_WORLD,tGripper.tframe,EGM_FRAME_TOOL,\x:=egm_minmax_lin1 \y:=egm_minmax_lin1 \z:=egm_minmax_lin1 \rx:=egm_minmax_rot1 \ry:=egm_minmax_rot1 \rz:=egm_minmax_rot1;
    ENDPROC
ENDMODULE