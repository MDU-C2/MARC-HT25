            ...
            for camera in result_config['cameras'].keys():
                ...
                if "intrinsics" in cam_info:
                    cam_data = {
                        ...
                        "image_size": cam_info["size"],
                        "reprojection_error": cam_info["reprojection_error"],
                    }
                    # Only add rvecs/tvecs if present
                    if "rvecs" in cam_info and "tvecs" in cam_info:
                        cam_data["rvecs"] = [rvec.tolist() for rvec in cam_info["rvecs"]]
                        cam_data["tvecs"] = [tvec.tolist() for tvec in cam_info["tvecs"]]

                    out_path = Path(self.dataset_path) / f"intrinsics_{cam_info['name']}.json"
                    with open(out_path, "w") as f:
                        json.dump(cam_data, f, indent=4)
                    print(f"Saved intrinsics + rvecs/tvecs for {cam_info['name']} to {out_path}")
